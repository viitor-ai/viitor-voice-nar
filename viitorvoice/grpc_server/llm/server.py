...
from viitorvoice.grpc_server.llm.servicer import ViiTorVoiceLLMServicer
from viitorvoice.grpc_server.proto import viitorvoice_llm_pb2_grpc as llm_pb2_grpc


LOGGER = logging.getLogger("viitorvoice.inference.grpc_server.llm")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="ViiTorVoice LLM gRPC v2 service.")
...
async def serve(config: V2RuntimeConfig) -> None:
    clear_proxies()
    runtime = LLMRuntime(config.service)
    await runtime.start()
    ...
    llm_pb2_grpc.add_ViiTorVoiceLLMServiceServicer_to_server(
        ViiTorVoiceLLMServicer(runtime, config.service),
        server,
    )
    address = f"{config.service.host}:{config.service.port}"
    server.add_insecure_port(address)
    await server.start()
    LOGGER.info("ViiTorVoice LLM gRPC v2 service listening on %s", address)
    try:
...
def main() -> None:
    ...
    config = V2RuntimeConfig.from_env(default_port=51052)
    ...
    try:
        ...
        LOGGER.info("ViiTorVoice LLM gRPC v2 service stopped")
